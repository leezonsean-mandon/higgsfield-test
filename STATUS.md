...
- Soul character **"Mandon"** (type soul_2) training started 14:47Z on all
  20 refs — soul_id `4a99edd9-e70a-4a3f-98cf-e758dc53f038` (~10 min).
...
## NEXT STEP
1. Poll `show_characters` action=status soul_id=4a99edd9-e70a-4a3f-98cf-e758dc53f038
   until status=ready (if this session ended before confirming, do it first).
2. On Mandon's calibration approval: 6× Soul 2.0 stills, 9:16, 2k, varied
...
3. Then first campaign brief (garment photos/URL + vibe) → package + quote →
   approval → generate → gallery + manifest.

Standing decisions: 9:16 default, formats photo_set/cinematic_video/try_on_video,
manual publishing only, quote before every batch. Balance 797.5 (Plus); verify
whether soul training charged anything via `balance`/`transactions` when it
finishes and update manifest credits.
